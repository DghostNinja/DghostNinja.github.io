...
I was recently using a coding agent(Opencode) to develop a plugin, and I suddenly got curious if there is a way for people without the necessary GPU to run their own model locally or in the cloud, then use with Opencode. The first thing I thought of was running a model locally using Ollama, but this won't fly well on CPU. 

First, I deployed a **Qwen2.5-Coder-3B-Instruct** on **Hugging face**. It had to be something light weight but can andle coding tasks with the free plan.

![alt](/assets/images/vLLM/v1.png)

*Remember what I said about running LLM models on CPU - slow as f*****
...
Now, let's cut the chase. Stay with me and don't zone out.
...
- On top menu, click on *Runtime* then *change runtime type*. Choose **T4 GPU** and connect.
...
Run the following commands one after the order (follow instruct well because you'd need to restart the service). Click on play on the play icon to run the command:
...
We are starting the tunnel first because, once the the vLLM starts running, we won't be able to run any other command till we stop the vLLM or append **&** to run in the background.
...
All you need to do is input your model's exposed url with model's into Oxium and you are good to go.
...
------
### Conclusion
Remember, you are on free plan. The notebook might timeout when you use for a long period, and you will need to wait for it to reset. So, switch the notebook off and disconnect runtime or just delete the session.
